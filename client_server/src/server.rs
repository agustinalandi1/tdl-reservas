...
async fn create_reservation(info: web::Json<(Usuario, String)>, sistema: web::Data<Arc<Sistema>>) -> impl Responder {
    let (usuario, date) = info.into_inner();
    let client_id = sistema.add_client(usuario.name.clone(), usuario.email.clone());
    let reservation_id = sistema.add_reservation(client_id, date.clone());
